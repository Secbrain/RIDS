	...
	size_t out_size)
{
	...
	if (strcmp(tokens[t0], "acl") == 0) {
		...
		else if (strcmp(tokens[t0 + 1], "raw") == 0){
			p.match.acl.ip_version = 2;
		}
		else {
			snprintf(out, out_size, MSG_ARG_NOT_FOUND,
				"ipv4 or ipv6");
	...
	struct table_rule_match *m)
{
	...
	if (strcmp(tokens[1], "acl") == 0) {
		...
		if(strcmp(tokens[4], "raw") == 0){
			//mytodo simplify
			//parse ip
			//struct in6_addr saddr, daddr;

			m->match.acl.ip_version = 2;
			uint32_t n_filter=60;
			uint32_t category_mask;
			if (parser_read_uint32(&category_mask,
				tokens[5]) != 0) {
					printf("category\n");
				snprintf(out, out_size, MSG_ARG_INVALID, "origin");
				return 0;
			}
			uint32_t i;
			for(i=0;i<n_filter;i++){
				if (parser_read_uint32(&m->match.raw.origin[i],
					tokens[6+i*2]) != 0) {
					printf("%d\n",i);
					snprintf(out, out_size, MSG_ARG_INVALID, "origin");
					return 0;
				}
				if (parser_read_uint32(&m->match.raw.mask[i],
					tokens[7+i*2]) != 0) {
					snprintf(out, out_size, MSG_ARG_INVALID, "mask");
					return 0;
				}
			}
			if (parser_read_uint32(&m->match.raw.priority,
				tokens[3]) != 0) {
				snprintf(out, out_size, MSG_ARG_INVALID, "priority");
				return 0;
			}
			if (parser_read_uint8(&m->match.raw.proto, tokens[6+n_filter*2]) != 0) {
				snprintf(out, out_size, MSG_ARG_INVALID, "proto");
				return 0;
			}
			if (parser_read_uint8(&m->match.raw.proto_mask, tokens[7+n_filter*2]) != 0) {
				snprintf(out, out_size, MSG_ARG_INVALID, "proto_mask");
				return 0;
			}
			m->match.raw.n_filter=n_filter;
			m->match.raw.category_mask=category_mask;
			return n_filter*2+8;
		}
		else{
			if (strcmp(tokens[4], "ipv4") == 0) {
