...
static const struct rte_acl_field_def table_acl_field_format_raw[] = {
	...
	struct table_params *params)
{
	...
	case TABLE_ACL:
	{
		...
		if (params->match.acl.ip_version == 1) {
		...
		} else if(params->match.acl.ip_version == 2){
			//mytodo
			//modify format
			memcpy(&pp.acl.field_format,
				&table_acl_field_format_raw,
				sizeof(table_acl_field_format_raw));
			pp.acl.n_rule_fields =
				RTE_DIM(table_acl_field_format_raw);
			printf("DEBUG:  copy raw rule.");
		}
		else {
			memcpy(&pp.acl.field_format,
