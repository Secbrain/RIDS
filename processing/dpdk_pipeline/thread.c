	...
	int add)
{
	...
	case TABLE_ACL:
		if (mh->match.acl.ip_version == 1){
			if (add) {
				ml->acl_add.category_mask=1;
		...
		//mytodo
		else if(mh->match.acl.ip_version == 2) {
			if (add) {
				ml->acl_add.field_value[0].value.u8 =
					mh->match.raw.proto;
				ml->acl_add.field_value[0].mask_range.u8 =
					mh->match.raw.proto_mask;
				uint32_t i;
				uint32_t n_filter=mh->match.raw.n_filter;
				ml->acl_add.category_mask=mh->match.raw.category_mask;
				for(i=0;i<n_filter;i++){
					ml->acl_add.field_value[i+1].value.u32=mh->match.raw.origin[i];
					ml->acl_add.field_value[i+1].mask_range.u32=mh->match.raw.mask[i];
				}
				// ml->acl_add.field_value[1].value.u32 =
				// 	mh->match.acl.ipv4.sa;
				// ml->acl_add.field_value[1].mask_range.u32 =
				...
				// ml->acl_add.field_value[4].mask_range.u16 =
				// 	mh->match.acl.dp1;
				printf("DEBUG:  add raw rule.");
				ml->acl_add.priority =
					(int32_t) mh->match.raw.priority;
			} 
			else {
				ml->acl_delete.field_value[0].value.u8 =
					mh->match.raw.proto;
